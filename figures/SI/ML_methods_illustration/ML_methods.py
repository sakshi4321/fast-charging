...
def plot_GP(n_points, ax):
    """Plot a Gaussian Process regression model's predictions and confidence intervals on the given axis.
    Parameters:
        - n_points (int): Number of data points to be generated for modeling.
        - ax (matplotlib.axes.Axes): Matplotlib axis object where the plot will be drawn.
    Returns:
        - None: The function modifies the provided axis object directly.
    Processing Logic:
        - Utilizes a Gaussian Process Regressor for fitting synthetic data generated by the function `f(x)`.
        - The 95% confidence interval is calculated and visualized based on the model's standard deviation.
        - The function prints the mean squared error (MSE) of the model's predictions on the generated data."""
    def f(x):
